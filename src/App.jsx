...
export default function FixedOpsTracker() {
  ...
  return <div className="min-h-screen bg-slate-50 p-4 text-slate-950 md:p-8"><div className="mx-auto max-w-7xl">
    ...
    {tab === "director" && <div className="space-y-6"><div className="grid gap-4 md:grid-cols-4"><Stat title="Group Repair Orders" value={qty(group.repairOrders)} /><Stat title="Group ELR" value={money(group.elr)} /><Stat title="Group Avg Gross Per RO" value={money(group.grossPerRo)} /><Stat title="Group Avg Parts/Labor" value={`${qty(group.partsToLabor*100,1)}%`} sub="Parts sale ÷ labor sale" /><Stat title="Group Labor Sale" value={money(group.labor)} sub={`Forecast: ${money(groupLaborForecast)}`} /><Stat title="Group Labor Gross" value={money(group.laborGross)} sub={`Forecast: ${money(groupLaborGrossForecast)}`}><p className="mt-2 text-sm font-semibold text-slate-500">Gross Profit %: {qty(groupLaborGrossPct,2)}%</p></Stat><div className="md:col-span-2 md:row-span-2"><Card className="h-full p-8"><div className="flex h-full min-h-[330px] flex-col items-center justify-center text-center"><p className="text-xl font-bold text-slate-500">Parts and Service Summary</p><div className="mt-6 w-full space-y-5 rounded-2xl bg-slate-100 px-6 py-6"><div><p className="text-sm font-bold uppercase text-slate-500">Total Sale</p><p className="mt-2 text-4xl font-extrabold">{money(group.labor+group.parts)}</p></div><div className="border-t pt-5"><p className="text-sm font-bold uppercase text-slate-500">Total Gross</p><p className="mt-2 text-4xl font-extrabold">{money(group.totalGross)}</p></div><div className="border-t pt-5"><p className="text-sm font-bold uppercase text-slate-500">Tracking End of Month</p><p className="mt-2 text-4xl font-extrabold">{money(groupSaleForecast)}</p></div><div className="border-t pt-5"><p className="text-sm font-bold uppercase text-slate-500">Tracking Gross End of Month</p><p className="mt-2 text-4xl font-extrabold">{money(groupGrossForecast)}</p></div></div></div></Card></div><Stat title="Group Parts Sale" value={money(group.parts)} sub={`Forecast: ${money(groupPartsForecast)}`} /><Stat title="Group Parts Gross" value={money(group.partsGross)} sub={`Forecast: ${money(groupPartsGrossForecast)}`}><p className="mt-2 text-sm font-semibold text-slate-500">Gross Profit %: {qty(groupPartsGrossPct,2)}%</p></Stat></div><Card className="p-5"><h2 className="mb-4 text-xl font-bold">GROUP OVERVIEW — Store Running Totals</h2><p className="mb-4 text-sm text-slate-500">Dealers are shown side by side in their own boxes so each store can compare performance against the group.</p><div className="overflow-x-auto"><div className="flex gap-4 pb-2">{stores.filter((s)=>allowedStores.includes(s)).map((s)=>{const st=totalsOf(visibleRows.filter((r)=>r.store===s));return <Card key={s} className="min-w-[260px] bg-slate-50 p-5 shadow-none"><div className="mb-4 flex items-center justify-between gap-3"><h3 className="text-lg font-bold">{s}</h3><span className="rounded-full bg-white px-3 py-1 text-xs font-semibold text-slate-600 shadow-sm">{qty(monthPercent,1)}% month</span></div><div className="space-y-3 text-sm"><div className="rounded-xl bg-white p-3 shadow-sm"><p className="text-slate-500">MTD ROs</p><p className="font-bold">{qty(st.repairOrders)}</p></div><div className="rounded-xl bg-white p-3 shadow-sm"><p className="text-slate-500">Hours</p><p className="font-bold">{qty(st.hours,1)}</p></div><div className="rounded-xl bg-white p-3 shadow-sm"><p className="text-slate-500">Labor Sales</p><p className="font-bold">{money(st.labor)}</p></div><div className="rounded-xl bg-white p-3 shadow-sm"><p className="text-slate-500">Parts Sales</p><p className="font-bold">{money(st.parts)}</p></div><div className="rounded-xl bg-white p-3 shadow-sm"><p className="text-slate-500">Labor Gross</p><p className="font-bold">{money(st.laborGross)}</p></div><div className="rounded-xl bg-white p-3 shadow-sm"><p className="text-slate-500">Parts Gross</p><p className="font-bold">{money(st.partsGross)}</p></div><div className="rounded-xl bg-white p-3 shadow-sm"><p className="text-slate-500">ELR</p><p className="mt-1 font-bold">{money(st.elr)}<Badge value={st.elr} benchmark={group.elr}/></p><p className="text-xs text-slate-500">Group ELR: {money(group.elr)}</p></div><div className="rounded-xl bg-white p-3 shadow-sm"><p className="text-slate-500">Gross Per RO</p><p className="mt-1 font-bold">{money(st.grossPerRo)}<Badge value={st.grossPerRo} benchmark={group.grossPerRo}/></p><p className="text-xs text-slate-500">Group Avg: {money(group.grossPerRo)}</p></div><div className="rounded-xl bg-white p-3 shadow-sm"><p className="text-slate-500">Parts/Labor</p><p className="mt-1 font-bold">{qty(st.partsToLabor*100,1)}%<Badge value={st.partsToLabor} benchmark={group.partsToLabor}/></p><p className="text-xs text-slate-500">Group Avg: {qty(group.partsToLabor*100,1)}%</p></div><div className="rounded-xl bg-white p-3 shadow-sm"><p className="text-slate-500">Month Progress</p><p className="font-bold">{qty(monthPercent,1)}%</p></div></div><Progress percent={monthPercent}/></Card>})}</div></div></Card></div>}
  </div></div>;
}
